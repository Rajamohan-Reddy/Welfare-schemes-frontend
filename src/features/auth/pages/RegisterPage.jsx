...
import {
  ...
  ArrowRight, ShieldCheck, Loader2, CheckCircle
} from "lucide-react";
...
const passwordStrength = (pw) => {
  ...
  if (pw.length < 10 || !/[A-Z]/.test(pw)) return { label: "Weak", color: "bg-orange-400", width: "w-2/4" };
  if (!/[^a-zA-Z0-9]/.test(pw)) return { label: "Good", color: "bg-yellow-400", width: "w-3/4" };
...
function FieldGroup({ label, error, children }) {
  return (
    <div className="space-y-1.5">
      <label className="block text-[11px] font-bold uppercase tracking-[0.12em] text-slate-500">{label}</label>
      {children}
      {error && (
        <p className="text-[11px] font-semibold text-rose-500 flex items-center gap-1 mt-1">
          <span className="h-1 w-1 rounded-full bg-rose-500 inline-block" /> {error}
        </p>
      )}
    </div>
  );
...
function InputWrap({ error, icon: Icon, children, className = "" }) {
  return (
    ...
    >
      {Icon && <Icon size={15} className="shrink-0 text-slate-400 group-focus-within:text-blue-500 transition mr-3" />}
      {children}
    </div>
  );
}
...
function RegisterPage() {
  ...
  const validateStep1 = () => {
    ...
    else if (!/^\d{10,12}$/.test(form.phoneNumber)) nextErrors.phoneNumber = "Enter a valid 10-digit mobile number.";
        ...
        <div className="flex items-center gap-2 mb-4">
          {[1, 2].map((s) => (
            <div key={s} className="flex items-center gap-2">
              ...
              <span className={`text-[10px] font-bold ${s === step ? "text-[#071A52]" : "text-slate-400"}`}>
        ...
        <form onSubmit={handleNext} className="space-y-4">
          ...
          <div className="pt-4 border-t border-slate-100 text-center">
            <p className="text-[13px] text-slate-500">
              Already registered?{" "}
              <Link to="/login" className="font-bold text-blue-600 hover:text-blue-800 transition hover:underline">
                Sign In →
              </Link>
            </p>
          </div>
      ...
      {step === 2 && (
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {strength && (
              <div className="mt-2 space-y-1">
                ...
                <p className={`text-[10px] font-bold ${
                  ...
                  strength.label === "Good" ? "text-yellow-600" : "text-orange-600"
                }`}>
          ...
          <div className="flex items-center gap-3 text-[11px] font-medium text-slate-400 bg-slate-50/70 border border-slate-200 rounded-2xl px-4 py-3">
            ...
            Your data is protected under AP Government Data Privacy Standards and never shared with third parties.
          </div>
