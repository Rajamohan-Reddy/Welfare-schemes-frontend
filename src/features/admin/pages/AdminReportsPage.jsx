...
import { useGetAuditLogsQuery } from "../../../store/services/reports.api";

function AdminReportsPage() {
  const { data: auditResponse, isLoading: loading } = useGetAuditLogsQuery();
  const auditLogs = auditResponse?.logs || auditResponse || [];
  ...
  const downloadAuditLogs = () => {
    if (!auditLogs.length) {
      toast.error("No audit logs available");
      return;
    }
    ...
    toast.success("Audit Logs Report Downloaded");
  };
      ...
      <section className="rounded-[32px] bg-slate-950 p-8 shadow-2xl">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-slate-300">
              Export audit and compliance reports generated from real system
              activity.
            </p>
          </div>

          <div className="rounded-3xl bg-white/10 px-5 py-4 text-sm font-semibold text-slate-100 shadow-inner">
            <span className="block text-slate-300">Available records</span>
      ...
      <Card className="space-y-5 rounded-[32px] p-6">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Download complete administrator and officer activity logs for
